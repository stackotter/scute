...
static delimiter *insert(cmark_syntax_extension *self, cmark_parser *parser,
    ...
    module = opener->inl_text;

    if (opener->inl_text->as.literal.len != closer->inl_text->as.literal.len)
        goto done;

    if (!cmark_node_set_type(module, CMARK_NODE_MODULE))
        goto done;

    cmark_node_set_syntax_extension(module, self);
    ...
    while (tmp) {
        ...
        cmark_node_append_child(module, tmp);
        tmp = next;
    }

    module->end_column = closer->inl_text->start_column + closer->inl_text->as.literal.len - 1;
...
static int can_contain(cmark_syntax_extension *extension, cmark_node *node,
    ...
    if (node->type != CMARK_NODE_MODULE)
        return false;
